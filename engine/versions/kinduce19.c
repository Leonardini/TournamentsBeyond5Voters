...
static int solve_one(int *out_nodes_lo) {
    ...
    /* The base-state LIST depends only on the ABSTRACT 5-vertex tournament
     * (mask bm), exactly as the count cache above does, so memoise it across a
     * batch.  Re-enumerating the sorted multisets every instance cost ~111ms at
     * n=15,K=5 while the search itself was ~14 nodes -- 99.99% pure overhead. */
    static int cached_bm = -1;
    static long long cached_nb = 0;
    if (bm != cached_bm) { cached_nb = base_states(bm, 1, bst, 4000000); cached_bm = bm; }
    long long nb = cached_nb;
