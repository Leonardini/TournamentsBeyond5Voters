...
static int classify_only = 0;
static long long base_dead_seed = 0, base_dead_top0 = 0, base_live = 0;
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--classify")) classify_only = 1;
    ...
    for (long long b = blo; b < bhi && !got && !aborted; b++) {
        ...
        if (classify_only) {
            int seed = init_domains(base_sz);
            int viable = 0;
            if (seed) for (int i = 0; i < K; i++) {
                if (ordv[i][0] != 0) continue;
                if (top0_r < 0) { viable = 1; break; }
                if (ordv[i][1] == top0_r || (top0_r2 >= 0 && ordv[i][1] == top0_r2)) { viable = 1; break; }
            }
            printf("CLASS %lld %s\n", b, !seed ? "dead_seed" : (viable ? "alive" : "dead_aut"));
            if (!seed) base_dead_seed++; else if (viable) base_live++; else base_dead_top0++;
            continue;
        }
        if (use_inc) {
            if (!init_domains(base_sz)) { base_dead_seed++; got = 0; }
            else {
                long long t0f = top0_fails, nd0 = nodes;
                got = dfs_inc(base_sz);
                if (top0_fails == t0f + 1 && nodes == nd0 + 1) base_dead_top0++;
                else base_live++;
            }
        }
        else         got = dfs();
    ...
    printf("BASECLASS total=%lld dead_at_seeding=%lld killed_by_aut=%lld alive=%lld\n",
           base_dead_seed + base_dead_top0 + base_live, base_dead_seed, base_dead_top0, base_live);
