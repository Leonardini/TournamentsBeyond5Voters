...
static int solve_one(int *out_nodes_lo) {
    ...
    /* The base-state LIST depends only on the ABSTRACT 5-vertex tournament, so
     * cache one list PER MASK across the batch.  A single-slot memo does NOT
     * work: auto-base picks a different LABELLED mask per instance even when the
     * abstract base tournament is the same, so the slot always missed.  At
     * n=15,K=5 re-enumerating cost ~111ms/instance against a ~14-node search. */
    static int (**bcache)[MAXK] = NULL;
    static long long *bcache_n = NULL;
    if (!bcache) {
        bcache   = calloc((size_t)1 << bNE, sizeof *bcache);
        bcache_n = calloc((size_t)1 << bNE, sizeof *bcache_n);
    }
    if (!bcache[bm]) {
        static int (*scratch)[MAXK] = NULL;
        if (!scratch) scratch = malloc(sizeof(int) * MAXK * 4000000);
        long long c = base_states(bm, 1, scratch, 4000000);
        bcache[bm] = malloc(sizeof(int) * MAXK * (c ? c : 1));
        memcpy(bcache[bm], scratch, sizeof(int) * MAXK * c);
        bcache_n[bm] = c;
    }
    int (*bst)[MAXK] = bcache[bm];
    long long nb = bcache_n[bm];
