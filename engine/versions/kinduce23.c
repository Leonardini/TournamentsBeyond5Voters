...
static int emit_all = 0;   /* --emit: with --count, print EVERY witness and keep going */
...
static int dfs_inc(int d) {
    ...
    if (d == n) { sols++; if (find_one) { print_and_verify(); return 1; } if (emit_all) print_and_verify(); return 0; }
...
static int dfs(void) {
    ...
    if (m == n) { sols++; if (find_one) { print_and_verify(); return 1; } if (emit_all) print_and_verify(); return 0; }
    build_suf();
...
int main(int argc, char **argv) {
    ...
    for (long long b = blo; b < bhi && !got && !aborted; b++) {
        ...
        for (int j = 0; j < base_sz; j++) placed |= 1ULL << base_v[j];
        if (base_sz == n) { sols++; if (find_one) { print_and_verify(); got = 1; } else if (emit_all) print_and_verify(); continue; }
        if (use_inc) got = init_domains(base_sz) ? dfs_inc(base_sz) : 0;
        else         got = dfs();
