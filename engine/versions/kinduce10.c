...
static int solve_one(int *out_nodes_lo) {
    ...
    nodes = 0; sols = 0; dom_calls = 0; dom_nodes = 0; mrv_fails = 0; bound_fails = 0; refine_tuples = 0; aborted = 0; tsp = 0;
    t0 = now();
    int got = 0;
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--count-base")) count_base_only = 1;
    ...
    if (count_base_only) {
        double ta = now();
        long long c = base_states(bm, 0, NULL, 0);     /* count, do not store */
        printf("BASECOUNT base_sz=%d mask=%d states=%lld enum_time=%.1fs\n",
               base_sz, bm, c, now() - ta);
        return 0;
    }
    ...
    printf("RESULT %s nodes=%lld sols=%lld base_states=%lld dom_calls=%lld dom_nodes=%lld mrv_fails=%lld bound_fails=%lld refine_tuples=%lld time=%.3fs\n",
           aborted ? "ABORTED" : (find_one ? (got ? "SAT" : "UNSAT") : "COUNTED"),
           nodes, sols, nb, dom_calls, dom_nodes, mrv_fails, bound_fails, refine_tuples, el);
    return 0;
}
