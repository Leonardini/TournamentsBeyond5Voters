...
static int solve_one(int *out_nodes_lo) {
    ...
    nodes = 0; sols = 0; dom_calls = 0; dom_nodes = 0; mrv_fails = 0; bound_fails = 0; refine_tuples = 0; fas_fails = 0; fas_tests = 0; DS = 0; aborted = 0; tsp = 0;
    t0 = now();
    int got = 0;
...
int main(int argc, char **argv) {
    ...
    printf("RESULT %s nodes=%lld sols=%lld base_states=%lld dom_calls=%lld dom_nodes=%lld mrv_fails=%lld bound_fails=%lld fas_fails=%lld/%lld refine_tuples=%lld time=%.3fs\n",
           aborted ? "ABORTED" : (find_one ? (got ? "SAT" : "UNSAT") : "COUNTED"),
           nodes, sols, nb, dom_calls, dom_nodes, mrv_fails, bound_fails, fas_fails, fas_tests, refine_tuples, el);
    return 0;
}
