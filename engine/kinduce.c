...
static long long nodes_at[MAXN];      /* search nodes by depth -- mean/max depth */
...
static int dfs_inc(int d) {
    ...
    if (d == n) { sols++; if (find_one) { print_and_verify(); return 1; }
    ...
    for (int v = 0; v < n; v++) {
        ...
        if (measure_dsz && d < MAXN) {
            /* The original dsz hook sat in the NON-incremental MRV loop, so it
             * never fired under --inc -- the path everyone uses.  domcnt is the
             * incremental path's own domain size, i.e. the branching factor. */
            int b = 0; long long x = c; while (x > 1 && b < 7) { x >>= 1; b++; }
            dsz_hist[d][b]++; dsz_sum[d] += c; dsz_cnt[d]++;
            if (c > dsz_max[d]) dsz_max[d] = c;
        }
...
static int dfs(void) {
    ...
    if (m == n) { sols++; if (find_one) { print_and_verify(); return 1; }
                  if (emit_all) print_and_verify(); return 0; }
    build_suf();
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--stats")) measure_dsz = 1;
    ...
    if (measure_dsz) {
        long long tot = 0, wsum = 0, mx = 0, bfmax = 0;
        for (int d = 0; d < MAXN; d++) {
            tot += nodes_at[d]; wsum += (long long)d * nodes_at[d];
            if (nodes_at[d]) mx = d;
            if (dsz_max[d] > bfmax) bfmax = dsz_max[d];
        }
        printf("SEARCHTREE nodes=%lld mean_depth=%.2f max_depth=%lld max_branching=%lld\n",
               tot, tot ? (double)wsum / tot : 0.0, mx, bfmax);
        printf("  depth: ");
        for (int d = 0; d < MAXN; d++) if (nodes_at[d])
            printf("%d:%lld(bf<=%lld) ", d, nodes_at[d], dsz_max[d]);
        printf("\n");
    }
