...
/* --emit: with --count, print EVERY witness and keep going instead of halting
 * on the first.  Used to enumerate the Paley(23)-v witness pool for the
 * arc-criticality study (WITNESSES_paley_arcrev.md).                        */
static int emit_all = 0;
/* --cover: settle ALL 2^n one-vertex extensions of the target in ONE search.
 * At a complete profile the achievable arc patterns between a NEW vertex and
 * the placed set are exactly the domain with the A/B requirement dropped, so OR
 * them into a 2^n bitset and stop the moment it fills.  Replaces the per-witness
 * (n+1)^k threshold grid and the 2^n cross-off list entirely.               */
static int cover_mode = 0;
static unsigned char *cov = NULL;
static long long cov_have = 0, cov_need = 0, cov_profiles = 0, cov_grids = 0;
/* The DFS emits profiles in near-lexicographic order, so consecutive ones are
 * nearly identical and add almost no new masks -- yet each still costs a full
 * (n+1)^k leaf sweep.  Skip ahead geometrically after a profile that adds
 * nothing, resetting the moment one pays.  Purely a scheduling device: it can
 * only DELAY coverage, never claim it, so an ALL verdict stays sound.       */
static long long cov_skip = 0, cov_skip_left = 0, cov_skip_cap = 4096;
static long long cov_caps_fired = 0;  /* a CAP stopped the search somewhere:
     * a profile was skipped, a base state was abandoned, or a time/node limit
     * fired.  If this is 0 and coverage is still short, the search EXHAUSTED
     * and the missing masks are genuinely unreachable -- a real candidate.
     * If it is nonzero, INCOMPLETE means only "the cap bit" and MUST be re-run
     * uncapped before anyone calls it a counterexample.                     */
static int cov_dump = 0;   /* print the UNCOVERED masks -- for a genuine
                            * candidate these are the prize: each names an
                            * order-(n+1) tournament with no realization.  */
 ...
 * anchoring one voter's top vertex at 0 stays WLOG (the break that --top0rr
 * applies, with its second-position refinement), and its residual
 * G_0 = Stab_Aut(0), cyclic of order
...
static void cov_rec(int i, uint64_t b0, uint64_t b1, uint64_t b2) {
    if (cov_have >= cov_need) return;
    if (i == K) {
        uint64_t mask;
        if (exact_margin) {
            uint64_t hi = eqm(b0, b1, b2, tHI), lo = eqm(b0, b1, b2, tLO);
            if (((hi | lo) & placed) != placed) return;   /* a vertex off-margin */
            mask = hi & placed;
        } else {
            mask = geq(b0, b1, b2, tHI) & placed;
        }
        size_t ix = (size_t)mask;
        if (!(cov[ix >> 3] & (1u << (ix & 7)))) {
            cov[ix >> 3] |= (unsigned char)(1u << (ix & 7)); cov_have++;
        }
        return;
    }
    for (int p = 0; p <= m; p++) {
        ...
        car2 = c1 & car; c1 ^= car;
        c2 ^= car2;
        cov_rec(i + 1, c0, c1, c2);
        if (cov_have >= cov_need || aborted) return;
    }
}
static void cover_collect(void) {
    cov_profiles++;
    if (cov_skip_left > 0) { cov_skip_left--; cov_caps_fired++; return; }
    long long before = cov_have;
    build_suf(); cov_grids++; cov_rec(0, 0, 0, 0);
    if (cov_have == before) {
        cov_skip = cov_skip ? cov_skip * 2 : 1;
        if (cov_skip > cov_skip_cap) cov_skip = cov_skip_cap;
    } else cov_skip = 0;
    cov_skip_left = cov_skip;
}
...
static int dfs_inc(int d) {
    ...
    if (per_base_nodes && (nodes & 0xFFF) == 0 && nodes - base_nodes0 >= per_base_nodes) {
        base_abort = 1; cov_caps_fired++; return 0;
    }
    if ((nodes & 0x3FFFF) == 0) {
        if (node_limit && nodes >= node_limit) { aborted = 1; cov_caps_fired++; return 0; }
        if (time_limit > 0 && now() - t0 > time_limit) { aborted = 1; cov_caps_fired++; return 0; }
        if (verbose) { fprintf(stderr, "  [%7.1fs] nodes=%-12lld depth=%2d sols=%lld\n",
                               now() - t0, nodes, d, sols); fflush(stderr); }
    }
    if (d == n) { sols++;
                  if (cover_mode) { cover_collect();
                      if (cov_have >= cov_need) aborted = 1; return 0; }
                  if (find_one) { print_and_verify(); return 1; }
                  if (emit_all) print_and_verify(); return 0; }
...
static int dfs(void) {
    nodes++;
    if (per_base_nodes && (nodes & 0xFFF) == 0 && nodes - base_nodes0 >= per_base_nodes) {
        base_abort = 1; cov_caps_fired++; return 0;               /* skip THIS base state only */
    }
    if ((nodes & 0x3FFFF) == 0) {
        if (node_limit && nodes >= node_limit) { aborted = 1; cov_caps_fired++; return 0; }
        if (time_limit > 0 && now() - t0 > time_limit) { aborted = 1; cov_caps_fired++; return 0; }
...
static void brec(int t, int lo) {
    if (t == K) {
        for (int e = 0; e < bNE; e++) {
            ...
            /* The supHI half was MISSING before 2026-09-04: --max-margin capped
             * arcs during insertion but never base-INTERNAL ones, so a base
             * state with an internal arc at K:0 was enumerated and never
             * rejected.  UNSAT stayed sound; a SAT would have needed an
             * independent histogram.  By the 3-cycle bound the gap is vacuous
             * on every host here, so no published verdict is affected.      */
            else { if (btarget[e] == tHI ? (bcnt_e[e] < tHI || bcnt_e[e] > supHI)
                                         : (bcnt_e[e] > tLO || bcnt_e[e] < K - supHI)) return; }
        }
        if (bmode == 1) {
    ...
    for (int p = (symbreak ? lo : 0); p < NBP; p++) {
        ...
        for (int e = 0; e < bNE; e++) {
            ...
            if (exact_margin) {
                ...
                if (btarget[e] == tHI) { if (c + rem < tHI || c > supHI) { ok = 0; break; } }
                else                   { if (c > tLO || c + rem < K - supHI) { ok = 0; break; } }
            }
        }
        if (!ok) continue;
...
static void cov_print_missing(int idx) {
    if (!cov_dump || cov_have >= cov_need) return;
    for (long long msk = 0; msk < cov_need; msk++)
        if (!(cov[msk >> 3] & (1u << (msk & 7)))) {
            printf("UNCOVERED %d mask=%lld bits=", idx, msk);
            for (int u = 0; u < n; u++) putchar((msk >> u) & 1 ? '1' : '0');
            putchar('\n');
        }
}
static void run_batch(void) {
    ...
    while (fgets(line, sizeof line, f)) {
        ...
        if (cover_mode) { memset(cov, 0, (size_t)((cov_need + 7) / 8));
                          cov_have = 0; cov_profiles = 0; cov_grids = 0;
                          cov_skip = 0; cov_skip_left = 0; cov_caps_fired = 0; }
        int got = solve_one(NULL);
        const char *v = aborted ? "ABORTED" : (got ? "SAT" : "UNSAT");
        if (aborted) nab++; else if (got) nsat++; else nunsat++;
        if (cover_mode)
            printf("COVER %d covered=%lld/%lld profiles=%lld grids=%lld %s exhausted=%d nodes=%lld time=%.3f\n",
                   idx, cov_have, cov_need, cov_profiles, cov_grids,
                   cov_have >= cov_need ? "ALL" : "INCOMPLETE",
                   cov_caps_fired == 0 ? 1 : 0, nodes, now() - t0),
            cov_print_missing(idx);
        else
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--emit")) emit_all = 1;
        else if (!strcmp(argv[i], "--cover")) { cover_mode = 1; find_one = 0; }
        else if (!strcmp(argv[i], "--cover-skip")) cov_skip_cap = atoll(argv[++i]);
        else if (!strcmp(argv[i], "--per-base-nodes")) per_base_nodes = atoll(argv[++i]);
        else if (!strcmp(argv[i], "--cover-dump")) cov_dump = 1;
    ...
    if (cover_mode) {
        cov_need = 1LL << n;
        cov = calloc((size_t)((cov_need + 7) / 8), 1);
        if (!cov) { fprintf(stderr, "cover: out of memory for 2^%d masks\n", n); return 1; }
        printf("  COVER MODE: settling all %lld extensions of this %d-vertex target\n",
               cov_need, n); fflush(stdout);
    }
    ...
    if (cover_mode)
        printf("COVER n=%d k=%d margin=%s covered=%lld/%lld profiles=%lld %s time=%.3f\n",
               n, K, exact_margin ? "exact1" : "majority", cov_have, cov_need, cov_profiles,
               cov_have >= cov_need ? "ALL_EXTENSIONS_INDUCIBLE" :
               (cov_caps_fired == 0 ? "INCOMPLETE_EXHAUSTED_REAL_CANDIDATE"
                                    : "INCOMPLETE_CAPPED_RERUN_UNCAPPED"), now() - t0),
        cov_print_missing(0);
    double el = now() - t0;
    if (measure_dsz) {
